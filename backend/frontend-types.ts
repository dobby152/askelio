...
export interface AskelioAPI {
  // Main processing endpoint
  processDocument(file: File, options?: ProcessingOptions): Promise<ApiResponse<ProcessDocumentResponse>>;
  
  // System endpoints
  getSystemStatus(): Promise<ApiResponse<SystemStatus>>;
  getCostStatistics(): Promise<ApiResponse<CostStatistics>>;
  getHealthStatus(): Promise<ApiResponse<HealthStatus>>;
  
  // Document management
  getDocuments(): Promise<ApiResponse<any[]>>;
  getDocument(id: number): Promise<ApiResponse<any>>;
}
...
export interface ProcessingProgress {
  stage: "uploading" | "ocr" | "llm" | "validation" | "storage" | "complete";
  message: string;
  percentage: number;
...
import { AskelioAPI, ProcessingOptions, ApiResponse, ProcessDocumentResponse } from './types';

const api: AskelioAPI = new AskelioAPIClient('http://localhost:8001');
...
const handleFileUpload = async (file: File) => {
  ...
  try {
    const response: ApiResponse<ProcessDocumentResponse> = await api.processDocument(file, options);
    ...
    if (response.success) {
      ...
      console.error('Processing failed:', response.error?.message);
    }
  } catch (error) {
