...
async def root():
    return {
        "message": "Askelio Invoice Processing API v2.2.0",
        "description": "Ultra Cost-Effective API with OCR + OpenRouter LLM (90% FREE processing)",
        "endpoints": {
...
@app.get("/test-cors")
async def test_cors():
    """Test CORS endpoint for frontend debugging"""
    return {
        "message": "CORS test successful",
        "timestamp": datetime.now().isoformat(),
        "server": "Askelio Backend v2.2.0",
        "cors_enabled": True
    }
...
async def upload_document_cost_effective(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Step 2: OpenRouter LLM processing
        llm_result = openrouter_llm.structure_invoice_data(
...
async def get_llm_statistics():
    ...
    try:
        stats = openrouter_llm.get_statistics()
        return {
            "status": "success",
            "statistics": stats,
...
async def get_llm_status():
    ...
    try:
        status = openrouter_llm.get_system_status()
        return {
            "status": "success",
            "llm_system": status,
...
async def get_system_status_unified():
    ...
    try:
        ...
        llm_status = openrouter_llm.get_system_status()
...
async def get_cost_statistics():
    ...
    try:
        ...
        llm_stats = openrouter_llm.get_statistics()
